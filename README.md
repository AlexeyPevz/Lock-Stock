...
### Верификация и генерация
- Ручная проверка текущего раунда: `/verify` (админ). Если факт-строки подтверждаются по Wikipedia API, раунд помечается `verified=1`.
- Фоновая генерация (по .env): `ENABLE_BG_GEN=1`, `BG_GEN_INTERVAL_SEC=600`. Генератор создаёт раунды, проверяет их через Wikipedia и сохраняет только прошедшие.
- Логи в админ-чат: задайте `ADMIN_LOG_CHAT_ID` — бот будет присылать уведомления об успешной/неуспешной проверке и ошибках фона.
